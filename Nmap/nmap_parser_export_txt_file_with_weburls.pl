		...
		if ($content =~ /<ports>(.+)<\/ports>/g) {      
			...
			while ($port_content =~ /<port ([^\n]+)<\/port>/g) {
			    ...
			    if ($sport_cont =~ /portid="([^"]+)"><state state="([^"]+)"/) {
			  	...
			  	if (($Pservice_name =~ /^https$/)||($Pservice_name =~ /^https-alt$/)) {
			  		if ($ssl != 1) {
			  	...
			  	elsif (($Pservice_name =~ /^http$/)||($Pservice_name =~ /^http-proxy$/)||($Pservice_name =~ /^http-alt$/)) {
			  		if ($ssl == 1) {
			  			# print "aaaaaaa\n$sport_cont\n\n";
			  			@protocols = qw(https);
			  		}
